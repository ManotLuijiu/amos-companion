...
test.describe("install.sh — Integration tests (Linux only)", { tag: ["@linux"] }, () => {
	test.beforeEach(() => {
		if (!IS_LINUX) {
	...
	test.describe("Environment variable overrides", () => {
		test("RELEASE_BASE can be overridden", () => {
			...
			console.log("  Sample output:", (result as string).split("\n").slice(0, 3).join(" "));
		});
		...
		test("INSTALL_DIR can be overridden", () => {
			...
			// Cleanup
			try { rmSync(tmpInstall, { recursive: true, force: true }); } catch (_e) { /* ignore cleanup errors */ }
		});
	...
	test.describe("Architecture detection", () => {
		...
		test("unknown arch exits with error", () => {
			...
			expect((result as string).includes("Unsupported architecture")).toBe(true);
		});
	});
	...
	test.describe("Desktop integration", () => {
		test("creates .desktop file content is valid", () => {
			...
			const desktopMatch = content.match(/cat > .*?amos-companion\.desktop.*?DESKTOP_EOF/s);
...
test.describe("Frontend — Linux install.sh download link", () => {
	...
	test("shows Linux install hint when on Linux", async ({ page }) => {
		...
		const isLinux = await page.evaluate(() => navigator.platform.includes("Linux"));
		if (isLinux) {
			// The frontend should surface guidance for Linux users
			// (This test verifies the placeholder/comment exists in source)
			const content = readFileSync(join(PROJECT_ROOT, "src-ui", "App.ts"), "utf8");
