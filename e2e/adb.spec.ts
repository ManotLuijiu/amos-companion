...
test.describe
	.serial("ADB Device Control", () => {
		test("should unlock device screen", async () => {
			...
			await new Promise<void>((resolve, _reject) => {
				exec(
					`${adb} -s ${DEVICE_SERIAL} shell input swipe 360 1000 360 200`,
					{ timeout: ADB_TIMEOUT },
					(err) => {
		...
		test("should perform tap action", async () => {
			...
			await new Promise<void>((resolve, reject) => {
				exec(
					`${adb} -s ${DEVICE_SERIAL} shell input tap 360 600`,
					{ timeout: ADB_TIMEOUT },
					(err) => {
						if (err) reject(err);
						else resolve();
		...
		test("should perform swipe action", async () => {
			...
			await new Promise<void>((resolve, reject) => {
				exec(
					`${adb} -s ${DEVICE_SERIAL} shell input swipe 360 1000 360 200 500`,
					{ timeout: ADB_TIMEOUT },
					(err) => {
						if (err) reject(err);
						else resolve();
		...
		test("should send text input", async () => {
			...
			await new Promise<void>((resolve, reject) => {
				exec(
					`${adb} -s ${DEVICE_SERIAL} shell input text hello`,
					{ timeout: ADB_TIMEOUT },
					(err) => {
						if (err) reject(err);
						else resolve();
...
test.describe
	.serial("ADB Screenshot & Screenrecord", () => {
		test("should capture screenshot", async () => {
			...
			await new Promise<void>((resolve, reject) => {
				exec(
					`${adb} -s ${DEVICE_SERIAL} shell screencap ${devicePath}`,
					{ timeout: ADB_TIMEOUT },
					(err) => {
						if (err) reject(err);
						else resolve();
			...
			await new Promise<void>((resolve, reject) => {
				exec(
					`${adb} -s ${DEVICE_SERIAL} pull ${devicePath} ${localPath}`,
					{ timeout: ADB_TIMEOUT },
					(err) => {
						if (err) reject(err);
						else resolve();
			...
			await new Promise<void>((resolve) => {
				exec(`${adb} -s ${DEVICE_SERIAL} shell rm ${devicePath}`, () =>
					resolve(),
				);
			});
		...
		test("should check screenrecord capability", async () => {
			...
			const result = await new Promise<string>((resolve, _reject) => {
				exec(
					`${adb} -s ${DEVICE_SERIAL} shell screenrecord --help`,
					{ timeout: ADB_TIMEOUT },
					(err, stdout) => {
						if (err) resolve("");
						else resolve(stdout);
